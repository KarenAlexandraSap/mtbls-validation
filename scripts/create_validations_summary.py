...
def create_summary_files(rules: dict[str, pd.Series]):
    ...
    violations_file = f"violations_{version}.json"
    ...
    versioned_content = {"validation_version": version, "created_at": now, "violations": violations_json, "summary": summary_json}
    with open(violations_file, "w") as f:
        json.dump(versioned_content, f, indent=4)

    with open("docs/MetaboLightsRules.md", "w") as f:
        
        f.write(f"| # |RULE ID              | TYPE                | TITLE                    | DESCRIPTION                  |\n")
        f.write(f"|---|---------------------|---------------------|--------------------------|------------------------------|\n")
        for idx, item in enumerate(violations):
            f.write(f"| {idx + 1} | {item.rule_id} | {item.type.value} | {item.title} | {item.description} |\n")
...
def get_items(rules):
    ...
    for rule_id, rule in rules.items():
        item = Violation(
            ...
            type=PolicyMessageType(str(rule["TYPE"].strip()) if rule["TYPE"] else PolicyMessageType.INFO))
        if len(rule_id) < 15:
            summary.append(item)
        else:
